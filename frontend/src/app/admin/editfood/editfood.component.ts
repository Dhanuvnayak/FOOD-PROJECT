...
export class EditfoodComponent implements OnInit {
  ...
  onSubmit(f: NgForm) {
    ...
    if (f.controls.foodpic.value) {
      ...
      // *************
      this.http.post<any>('http://localhost:10000/admin/editfoodwithimage', formData).subscribe(
        (res) => {
      ...
      this.http.get<any>('http://localhost:10000/admin/editfoodwithoutimage?id=' + this.id + '&foodname=' + this.pn + '&foodprice=' + this.pp
      ).subscribe(
        (res) => {
          this.adminService.avail = true;
          this.adminService.msg = "Successfully Edited a food!!!"
